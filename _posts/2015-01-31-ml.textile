...
category: math
---
p<>. First keep in mind that a machine learning problem is the mathematical abstraction of a real-world problem. So, to solve a real-world problem correctly one should proceed in two steps. Make an appropriate abstraction, say the machine learning problem first, and than solve this problem. Though the former is the prerequisite, it is a rather domain specific consideration, so this article shall starts from the more general step 2.
...
p<>. where !/images/ml/CodeCogsEqn-3.gif!. So the response is Normal distributed which gives an identical link function:

!{display: block;margin-left: auto;margin-right: auto;width: 23%;height: auto;}/images/ml/CodeCogsEqn.gif!

p<>. While in the binary classification problem, the response is Bernoulli distributed, and a __logit__ link function would be appropriate:
...
p<>. Sigmoid function is differentiable and has a range from 0 to !http://latex.codecogs.com/gif.latex?&plus;\infty!, making it suitable for a scoring binary classifier.
...
p<>. Ignoring the constant terms, maximizing the log-likelihood equals to minimizing empirical risk !http://latex.codecogs.com/gif.latex?\inline&space;\sum_{i=1}^{N}\left&space;(&space;y_{i}-f\left&space;(&space;x_{i}&space;\right&space;)&space;\right&space;)^{2}!, in terms of square error.
...
p<>. Obviously, they are of the same form, and the predictors __f(x)__ just differ by a constant factor.
